...
public class App {
    ...
    public static void main(String[] args) {
        ...
        var x = new Hero.Builder(Profession.MAGE, "emre");
        x = x.withArmor(Armor.CHAIN_MAIL);
        x = x.withHairColor(HairColor.BLACK);
        Hero myHero = x.build();
        System.out.println(myHero.toString());
        Hero mage =
                new Hero.Builder(Profession.MAGE, "Riobard").withHairColor(HairColor.BLACK)
                        .withWeapon(Weapon.DAGGER).withArmor(Armor.CHAIN_MAIL).build();
